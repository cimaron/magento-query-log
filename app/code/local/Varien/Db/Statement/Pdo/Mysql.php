...
class Varien_Db_Statement_Pdo_Mysql extends Zend_Db_Statement_Pdo
{
    ...
    public function _execute(array $params = null)
    {
		...
		Mage::dispatchEvent('varien_db_adapter_query_before', array('stmt' => $this->_stmt, 'params' => $params));

        if ($specialExecute) {
            $result = $this->_executeWithBinding($params);
        } else {
            $result = parent::_execute($params);
        }
		
		Mage::dispatchEvent('varien_db_adapter_query_after', array('stmt' => $this->_stmt, 'params' => $params));
